...
static int handle_api_register(struct MHD_Connection *connection, const unsigned char *body, size_t body_len) {
    ...
    free(tmp);
    if (strlen(username)==0 || strlen(password)==0) return respond_json(connection,"{"status":"error","message":"username/password required"}",MHD_HTTP_BAD_REQUEST);
    sqlite3_stmt *stmt; const char *sql="INSERT INTO users (username,email,password,room,role) VALUES (?, ?, ?, ?, 'student');";
    ...
    sqlite3_bind_text(stmt,1,username,-1,SQLITE_TRANSIENT); sqlite3_bind_text(stmt,2,email,-1,SQLITE_TRANSIENT); sqlite3_bind_text(stmt,3,password,-1,SQLITE_TRANSIENT); sqlite3_bind_text(stmt,4,room,-1,SQLITE_TRANSIENT);
...
static int handle_api_login(struct MHD_Connection *connection, const unsigned char *body, size_t body_len) {
    ...
    free(tmp);
    if (strlen(username)==0 || strlen(password)==0) return respond_json(connection,"{"status":"error","message":"username/password required"}",MHD_HTTP_BAD_REQUEST);
    sqlite3_stmt *stmt; const char *sql="SELECT id,password,role FROM users WHERE username = ? OR email = ?;";
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) != SQLITE_OK) return respond_json(connection,"{"status":"error","message":"db error"}",MHD_HTTP_INTERNAL_SERVER_ERROR);
    sqlite3_bind_text(stmt,1,username,-1,SQLITE_TRANSIENT); sqlite3_bind_text(stmt,2,username,-1,SQLITE_TRANSIENT);
    ...
    int user_id = sqlite3_column_int(stmt,0); const unsigned char *dbpass = sqlite3_column_text(stmt,1); const unsigned char *dbrole = sqlite3_column_text(stmt,2);
...
static int handle_api_complaints_get(struct MHD_Connection *connection, const char *token) {
    ...
    char json[20000]; strcpy(json,"{"status":"ok","complaints":["); int first=1;
    int rc2;
    while ((rc2 = sqlite3_step(qstmt)) == SQLITE_ROW) {
        ...
        char item[2048];
        snprintf(item,sizeof(item),"{"id":%d,"user":"%s","title":"%s","type":"%s","description":"%s","filename":"%s","status":"%s","created":"%s"}", cid, uname? (const char*)uname:"", title? (const char*)title:"", ctype? (const char*)ctype:"", desc? (const char*)desc:"", fname? (const char*)fname:"", status? (const char*)status:"", created? (const char*)created:"");
        strcat(json,item);
    }
...
static int answer_to_connection(void *cls, struct MHD_Connection *connection, const char *url, const char *method, const char *version, const char *upload_data, size_t *upload_data_size, void **con_cls) {
    if (strcmp(method,"GET")==0) {
        ...
        if (strncmp(url,"/static/",8)==0) {
            char path[512]; snprintf(path,sizeof(path),".%s",url); const char *mime="text/plain"; if (strstr(path,".css")) mime="text/css"; if (strstr(path,".js")) mime="application/javascript"; if (file_exists(path)) return send_file(connection,path,mime);
        }
...
int main(int argc, char **argv) {
    char *port_env = getenv("PORT"); int PORT = port_env ? atoi(port_env) : 8080;
    ensure_uploads_dir();
