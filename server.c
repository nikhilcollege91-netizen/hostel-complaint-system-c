/* server.c
   ...
   - Uses libmicrohttpd and sqlite3
   - Reads PORT from environment (default 10000)
...
static int handle_api_complaint_update(struct MHD_Connection *connection, const unsigned char *body, size_t body_len, const char *token) {
    ...
    char *tmp = malloc(body_len + 1);
    if (!tmp) return respond_json(connection, "{\"status\":\"error\",\"message\":\"alloc\"}", MHD_HTTP_INTERNAL_SERVER_ERROR);
    memcpy(tmp, body, body_len); tmp[body_len] = '\0';
    char *p = strtok(tmp, "&");
    while (p) {
...
static struct connection_info_struct *connection_info_create(void) {
    ...
    if (!ci) return NULL;
    ci->buffer = NULL; ci->size = 0; return ci;
}
static void connection_info_free(struct connection_info_struct *ci) { if (!ci) return; if (ci->buffer) free(ci->buffer); free(ci); }
            ...
            if (*upload_data_size) {
                ...
                *upload_data_size = 0;
                ...
                if (strcmp(url, "/api/register") == 0) {
                    ...
                    connection_info_free(ci); *con_cls = NULL; return ret;
